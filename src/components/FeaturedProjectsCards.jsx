...
const FeaturedProjectsCards = ({ data, cards = [] }) => {
  ...
  <section className="py-16 sm:py-24 bg-white text-[#111312] relative overflow-hidden">
    <div className="max-w-7xl mx-auto px-6">
            ...
            >
              ...
              <div className="relative z-20 h-full p-7 sm:p-8 flex flex-col justify-between text-right">
                <div className="space-y-4 pt-2">
                  ...
                  <p className="text-white/90 text-sm sm:text-base leading-relaxed font-medium drop-shadow-sm line-clamp-3">
                    {card.description}
                  </p>
                </div>
                <div className="flex justify-end pt-4">
                  <button
                    type="button"
                    className="bg-[#D4E128] hover:bg-[#c2ce23] text-[#1E201E] font-bold text-sm px-6 py-2.5 rounded-full transition-all duration-300 shadow-lg hover:scale-105 active:scale-95 cursor-pointer"
                  >
                    {card.buttonText}
                  </button>
                </div>
      ...
      {/* Blog Details Modal for Home Featured Cards */}
      <BlogDetailsModal
        blog={selectedBlog}
        isOpen={!!selectedBlog}
